...
- Create the chore doc in the `project/chores/` directory with filename: `chore-{descriptive-name}.md`
  - Replace `{descriptive-name}` with a short, descriptive name based on the chore (e.g., "update-readme", "fix-tests", "refactor-auth")
- IMPORTANT: Chores don't need implementation plans. Document the chore, execute it, mark done.
