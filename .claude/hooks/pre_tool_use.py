...
def is_env_file_access(tool_name, tool_input):
    ...
    if tool_name in ['Read', 'Edit', 'MultiEdit', 'Write', 'Bash']:
        ...
        if tool_name in ['Read', 'Edit', 'MultiEdit', 'Write']:
            ...
            if '.env' in file_path and not file_path.endswith('.env.sample') and not file_path.endswith('.env.example'):
                return True

        # Check bash commands for .env file access
        elif tool_name == 'Bash':
            command = tool_input.get('command', '')
            # Pattern to detect .env file access (but allow .env.sample/.env.example)
            env_patterns = [
                r'\b\.env\b(?!\.sample)(?!\.example)',  # .env but not .env.sample/.env.example
